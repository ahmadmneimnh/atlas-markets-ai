    ...
    <div className="space-y-6 animate-fade-up">
      ...
      <Card className="p-4">
        <form className="flex flex-wrap items-end gap-4">
          <label className="text-xs">
            <span className="mb-1.5 block uppercase tracking-wider text-ink-faint">
              Minimum AI score
            </span>
            <input
              type="number"
      ...
      {rows.length === 0 ? (
        <Unavailable
          ...
          reason="Lower the minimum score or widen the asset class filter."
        />
      ) : (
        <Card className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-sm">
            ...
            <tbody className="divide-y divide-glass-border/40">
              {rows.map((r) => (
                <tr
                  key={`${r.score.ref.kind}-${r.score.ref.symbol}`}
                  className="transition-colors hover:bg-glass"
                >
                  <td className="p-4">
                    <Link
                      href={`/asset/${r.score.ref.kind}/${r.score.ref.symbol}`}
                      className="block"
                    >
                  ...
                  <td className="tnum p-4">
                    {r.quote ? (
                      `$${formatPrice(r.quote.price)}`
                    ) : (
                  ...
                  <td className="p-4">
                    {r.quote ? (
                      <Delta value={r.quote.changePercent} />
                    ) : (
                      <span className="text-ink-faint">—</span>
                    )}
                  </td>
                  ...
                  <td className="p-4">
                    <RecommendationBadge recommendation={r.score.recommendation} size="sm" />
                  </td>
