...
/**
 * The full market view: scored assets, movers, heatmap, trending, calendars and
 * news.
 *
 * Extracted from what used to be the homepage so that `/stocks` and `/crypto`
 * can each render it scoped to one asset class. Nothing was dropped in the move
 * — the sections are the same, the data path is the same, and passing
 * `kind: 'all'` reproduces the original page exactly.
 */
export async function MarketSection({ kind = 'all' }: { kind?: 'equity' | 'crypto' | 'all' }) {
  const providers = configuredProviders();
  const hasEquityProvider = providers['finnhub'] || providers['twelvedata'] || providers['polygon'];

  // Crypto works with no credentials at all (CoinGecko public + Binance public),
  // so a fresh clone is still useful. Equities need a key.
  const universe =
    kind === 'crypto'
      ? CRYPTO_UNIVERSE
      : kind === 'equity'
  ...
  // An equity-only view with no equity provider would score nothing at all; the
  // banner below explains that rather than rendering an unexplained empty page.
  ...
  // Panels are fetched only where they are relevant to the class on screen.
  // Fear & Greed is a crypto index and the earnings calendar is an equity
  // concern; showing either under the wrong heading would misrepresent its
  // scope, and fetching it would spend quota for something never rendered.
  const wantsCrypto = kind !== 'equity';
  const wantsEquity = kind !== 'crypto';

  const [fearGreed, earnings, economic, headlines] = await Promise.all([
    wantsCrypto ? marketWide.fearGreed() : Promise.resolve(null),
    wantsEquity ? marketWide.earnings(isoDay(0), isoDay(7)) : Promise.resolve(null),
    wantsEquity ? marketWide.economicCalendar(isoDay(0), isoDay(7)) : Promise.resolve(null),
    market.news(null, 8),
  ]);
