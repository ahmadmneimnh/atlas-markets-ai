...
function classify(text: string): Classification {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    for (let j = Math.max(0, i - 3); j < i; j++) {
      if (NEGATORS.has(words[j] as string)) {
        sign = -1;
        break;
      }
    }
  ...
  score(ctx: AssetContext): FactorOutcome {
    ...
    if (totalWeight === 0) {
      return {
        factor: 'news',
        unavailable: true,
        reason: 'all articles fell outside the recency window',
      };
    }
    ...
    const signals: Signal[] = [
      {
        ...
        direction:
          positive > negative * 1.5 ? 'bullish' : negative > positive * 1.5 ? 'bearish' : 'neutral',
        weight: 0.5,
    ...
    for (const n of notable
      .sort((a, b) => Math.abs(b.c.magnitude) - Math.abs(a.c.magnitude))
      .slice(0, 4)) {
      signals.push({
