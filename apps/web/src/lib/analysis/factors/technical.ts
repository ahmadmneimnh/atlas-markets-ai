  ...
  score(ctx: AssetContext): FactorOutcome {
    ...
    if (cross) {
      contributions.push({
        ...
        signal: {
          label:
            cross === 'golden' ? 'Golden cross (SMA50 × SMA200)' : 'Death cross (SMA50 × SMA200)',
          value: 'today',
    ...
    if (ich) {
      ...
      contributions.push({
        ...
        signal: {
          ...
          direction:
            ich.position === 'above' ? 'bullish' : ich.position === 'below' ? 'bearish' : 'neutral',
          weight: 0.08,
          source: src,
        },
      });
    ...
    if (range) {
      ...
      contributions.push({
        ...
        signal: {
          ...
          direction:
            range.positionInRange > 0.75
              ? 'bullish'
              : range.positionInRange < 0.25
                ? 'bearish'
                : 'neutral',
          weight: 0.08,
          source: src,
    ...
    if (contributions.length === 0) {
      return {
        factor: 'technical',
        unavailable: true,
        reason: 'no indicator had sufficient history',
      };
    }
    ...
    return {
      ...
      signals: contributions.sort((x, y) => y.weight - x.weight).map((c) => c.signal),
    };
  },
};
