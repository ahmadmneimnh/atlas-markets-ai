...
import { Card, SectionTitle, Unavailable, Delta, formatPrice, Provenance } from '@/components/ui';
...
export default async function DashboardPage() {
  ...
  const topSells = [...byScore].reverse().filter((s) => s.score.score <= 45).slice(0, 4);
  ...
  const losers = movers.filter((s) => s.quote!.changePercent < 0).reverse().slice(0, 5);
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <section>
              ...
              <Card className="divide-y divide-glass-border/50">
                {gainers.length === 0 ? (
                  <p className="p-5 text-sm text-ink-faint">No positive movers in the tracked universe.</p>
                ) : (
            ...
            <section>
              ...
              <Card className="divide-y divide-glass-border/50">
                {losers.length === 0 ? (
                  <p className="p-5 text-sm text-ink-faint">No negative movers in the tracked universe.</p>
                ) : (
