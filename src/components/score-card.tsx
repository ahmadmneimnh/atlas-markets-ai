...
import {
  Card, RecommendationBadge, ScoreDial, Delta, DirectionDot,
  Provenance, formatPrice, SectionTitle,
} from './ui';
      ...
      <Card hover className="h-full p-5">
        <div className="mb-4 flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="truncate text-base font-semibold tracking-tight">{score.ref.symbol}</span>
        ...
        <div className="mt-4 border-t border-glass-border/60 pt-3">
          <div className="flex items-center justify-between text-[11px]">
            ...
            <span className={clsx('tnum font-medium', score.confidence < 40 ? 'text-warn' : 'text-ink-muted')}>
              {score.confidence}%
            </span>
          </div>
          <div className="mt-1.5 h-1 overflow-hidden rounded-full bg-glass-strong">
            <div
              className={clsx('h-full rounded-full', score.confidence < 40 ? 'bg-warn/70' : 'bg-gold/70')}
...
export function ScoreExplanation({ score }: { score: AssetScore }) {
  return (
    <div className="space-y-6">
      ...
      {/* Why */}
      <div>
        <SectionTitle hint="ranked by contribution to the final score">Why this rating</SectionTitle>
          ...
          {score.breakdown.map((b) => (
            <details key={b.factor} className="group">
              ...
              <div className="space-y-2 bg-canvas-sunken/40 px-4 pb-4 pt-1">
                ...
                {b.signals.map((s, i) => (
                  <div key={i} className="flex items-start gap-3 rounded-lg border border-glass-border/40 p-2.5">
      ...
      {score.omitted.length > 0 ? (
        <div>
          <SectionTitle hint="excluded from the score, not estimated">Factors not scored</SectionTitle>
          <Card className="divide-y divide-glass-border/50">
            {score.omitted.map((o) => (
