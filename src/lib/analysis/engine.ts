import type {
  AssetContext, AssetScore, FactorScorer, Recommendation,
  ScoreBreakdown, Signal, FactorOutcome,
} from './types';
...
export async function scoreAsset(
  ...
  const omitted = outcomes.filter(isUnavailable).map((o) => ({ factor: o.factor, reason: o.reason }));
