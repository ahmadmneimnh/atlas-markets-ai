...
export function macd(closes: number[], fast = 12, slow = 26, signalPeriod = 9): MacdResult | undefined {
...
export function maCross(closes: number[], fast = 50, slow = 200): CrossSignal {
  ...
  if (fastNow === undefined || slowNow === undefined || fastPrev === undefined || slowPrev === undefined) {
    return null;
  }
...
export function supportResistance(
  ...
  return {
    support: cluster(support).filter((l) => l < price).slice(-3),
    resistance: cluster(resistance).filter((l) => l > price).slice(0, 3),
  };
}
...
export interface TrendResult {
  direction: Trend;
  /** The moving averages the classification was based on. */
  ema20: number | undefined;
  ema50: number | undefined;
  ema200: number | undefined;
  /** How the label was reached, for display next to it. */
  basis: string;
}

/**
 * Trend from moving-average stacking rather than from a single slope.
 *
 * "Price above the 200-day" alone flips on every wick near the line; requiring the
 * averages to be ordered as well is what separates a trend from a bounce. Whichever
 * averages the history supports are used, and the basis string says which — a
 * 90-bar series cannot speak to the 200-day and should not pretend to.
 */
export function trendDirection(closes: number[]): TrendResult | undefined {
  ...
  if (e20 !== undefined) {
    checks.push(price > e20);
    parts.push(`price ${price > e20 ? 'above' : 'below'} EMA20`);
  }
  if (e20 !== undefined && e50 !== undefined) {
    checks.push(e20 > e50);
    parts.push(`EMA20 ${e20 > e50 ? 'above' : 'below'} EMA50`);
  }
  if (e50 !== undefined && e200 !== undefined) {
    checks.push(e50 > e200);
    parts.push(`EMA50 ${e50 > e200 ? 'above' : 'below'} EMA200`);
  }
  if (e200 !== undefined) {
    checks.push(price > e200);
    parts.push(`price ${price > e200 ? 'above' : 'below'} EMA200`);
  }
  if (checks.length === 0) return undefined;

  const bullish = checks.filter(Boolean).length;
  const ratio = bullish / checks.length;

  const direction: Trend =
    ratio === 1 ? 'strong_uptrend'
    : ratio >= 0.6 ? 'uptrend'
    : ratio > 0.4 ? 'sideways'
    : ratio > 0 ? 'downtrend'
    : 'strong_downtrend';
...
export interface VolumeTrendResult {
  ...
  /** Recent average volume divided by the earlier average. */
  ratio: number;
...
/**
 * Whether participation is expanding or drying up: the last `recent` bars' average
 * volume against the `baseline` bars before them.
 *
 * Returns undefined when the provider reports no volume at all (CoinGecko's /ohlc),
 * because a ratio computed from zeros would read as "volume collapsed" rather than
 * "volume unknown".
 */
export function volumeTrend(
  ...
  recent = 5,
  baseline = 20,
): VolumeTrendResult | undefined {
  if (candles.length < recent + baseline) return undefined;

  const recentBars = candles.slice(-recent);
  const baselineBars = candles.slice(-(recent + baseline), -recent);

  const mean = (bars: Candle[]) => bars.reduce((sum, c) => sum + c.volume, 0) / bars.length;
  const recentAverage = mean(recentBars);
  const baselineAverage = mean(baselineBars);
  if (baselineAverage <= 0 || recentAverage <= 0) return undefined;

  const ratio = recentAverage / baselineAverage;
  return {
    direction: ratio >= 1.2 ? 'rising' : ratio <= 0.8 ? 'falling' : 'flat',
    ratio,
    recentAverage,
    baselineAverage,
  };
