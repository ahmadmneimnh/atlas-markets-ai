...
interface Ticker24 {
  ...
  highPrice: string; lowPrice: string; openPrice: string; prevClosePrice: string;
  volume: string; closeTime: number;
}
...
export const binance: Provider = {
  ...
  async ohlcv(symbol: string, interval, limitCount: number): Promise<ProviderResult<OhlcvSeries>> {
    try {
      ...
      return ok({ symbol: symbol.toUpperCase(), interval, candles, source: 'binance', asOf: new Date() });
    } catch (e) {
      return classifyError(e);
    }
  },
