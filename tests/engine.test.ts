...
function fixed(factor: FactorScorer['factor'], weight: number, score: number, confidence = 1): FactorScorer {
  return {
    factor, weight,
    score: (): FactorOutcome => ({
      factor, score, confidence,
      signals: [{ label: `${factor} signal`, value: String(score), direction: 'bullish', weight: 1, source: 'test' }],
    }),
  };
}
...
describe('weighting', () => {
  it('computes a weighted mean when every factor is available', () => {
    const scorers = [fixed('technical', 30, 80), fixed('fundamental', 30, 60), fixed('news', 40, 50)];
  ...
  it('renormalizes weight away from a missing factor instead of scoring it neutral', async () => {
    ...
    const scorers = [fixed('technical', 30, 80), fixed('fundamental', 30, 80), missing('social', 40)];
  ...
  it('reports effective weights that sum to 100 after renormalization', async () => {
    const scorers = [fixed('technical', 30, 70), fixed('fundamental', 30, 40), missing('macro', 10), missing('social', 10)];
    const r = await scoreAsset(ctx, scorers);
...
describe('confidence', () => {
  it('is independent of score', async () => {
    ...
    const strong = await scoreAsset(ctx, [fixed('technical', 30, 80, 1.0), fixed('fundamental', 30, 80, 1.0)]);
    const weak = await scoreAsset(ctx, [fixed('technical', 30, 80, 0.3), fixed('fundamental', 30, 80, 0.3)]);
...
describe('refusal to guess', () => {
  it('throws rather than scoring when coverage is below the floor', async () => {
    ...
    const scorers = [
      fixed('risk', 5, 90),
      missing('technical', 30), missing('fundamental', 30),
      missing('news', 15), missing('social', 10), missing('macro', 10),
    ];
  ...
  it('throws when no factor is available at all', async () => {
    await expect(scoreAsset(ctx, [missing('technical', 30)])).rejects.toThrow(InsufficientDataError);
  });
...
describe('resilience', () => {
  it('degrades a throwing scorer to an unavailable factor', async () => {
    const exploding: FactorScorer = {
      factor: 'news', weight: 15,
      score: () => { throw new Error('provider exploded'); },
    };
    const r = await scoreAsset(ctx, [fixed('technical', 30, 70), fixed('fundamental', 30, 70), exploding]);
...
describe('explanation', () => {
  it('cites sources and ranks reasons by contribution', async () => {
    const scorers = [fixed('technical', 30, 85), fixed('fundamental', 30, 75), fixed('risk', 5, 60)];
    const r = await scoreAsset(ctx, scorers);
