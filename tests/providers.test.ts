...
describe('finnhub adapter', () => {
  it('normalizes the c/d/dp/h/l/o/pc quote shorthand', async () => {
    mockFetch({ c: 185.42, d: 2.13, dp: 1.16, h: 186.1, l: 183.5, o: 184, pc: 183.29, t: 1735689600 });
...
describe('alpha vantage adapter', () => {
  ...
  it('parses string-typed OHLC values and sorts oldest-first', async () => {
    mockFetch({
      'Time Series (Daily)': {
        ...
        '2025-01-03': { '1. open': '184.0', '2. high': '186.1', '3. low': '183.5', '4. close': '185.4', '5. volume': '48000000' },
        '2025-01-02': { '1. open': '182.0', '2. high': '184.0', '3. low': '181.0', '4. close': '183.3', '5. volume': '52000000' },
      },
    });
    const { alphavantage } = await import('@/lib/providers/equity/alphavantage');
  ...
  it('skips malformed bars instead of emitting NaN candles', async () => {
    mockFetch({
      'Time Series (Daily)': {
        '2025-01-02': { '1. open': '182.0', '2. high': '184.0', '3. low': '181.0', '4. close': '183.3', '5. volume': '52000000' },
        '2025-01-03': { '1. open': 'None', '2. high': '-', '3. low': '', '4. close': '', '5. volume': '' },
      },
    });
    const { alphavantage } = await import('@/lib/providers/equity/alphavantage');
...
describe('binance adapter', () => {
  it('parses the all-strings ticker payload', async () => {
    mockFetch({
      symbol: 'BTCUSDT', lastPrice: '95432.10', priceChange: '1234.50',
      priceChangePercent: '1.31', highPrice: '96000.00', lowPrice: '94000.00',
      openPrice: '94197.60', prevClosePrice: '94197.60', volume: '12345.678',
    ...
    mockFetch([
      {
        ...
        current_price: 95432.1, price_change_24h: 1234.5, price_change_percentage_24h: 1.31,
        high_24h: 96000, low_24h: 94000, total_volume: 45000000000, market_cap: 1890000000000,
        circulating_supply: 19800000, total_supply: 21000000, max_supply: 21000000,
        market_cap_rank: 1, last_updated: '2025-01-01T00:00:00.000Z',
      },
    ]);
...
describe('credential hygiene', () => {
  ...
  it('strips the credential from HttpError.url and .message', async () => {
    ...
    const err = new HttpError(401, 'nope', 'https://finnhub.io/api/v1/quote?symbol=AAPL&token=sk-secret-123');
  ...
  it('leaves non-credential query parameters untouched', async () => {
    ...
    const clean = scrubUrl('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin');
    expect(clean).toBe('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin');
  });
});
